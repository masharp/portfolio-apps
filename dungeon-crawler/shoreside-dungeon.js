...
const Map = {
  ...
  init: function init() {
    this.display = new ROT.Display({ fontSize: 18, bg: 'white', fg: '#006591' });
  ...
  _generateMap: function _generateMap() {
    const rogue = new ROT.Map.Rogue();
    const freeCells = [];

    const rogueCallback = function rogueCallback(x, y, value) {
  ...
  _generatePlayer: function _generatePlayer(freeCells) {
  ...
  _generateMonsters: function _generateMonsters(freeCells) {
    const monsters = [];
    const monsterTypes = [];

    for (let i = 0; i < 15; i++) {
      const type = ROT.RNG.getWeightedValue(MONSTER_TYPES.Weight);
  ...
  _generatePotions: function _generatePotions(freeCells) {
    for (let i = 0; i < 7; i++) {
      const index = Math.floor(ROT.RNG.getUniform() * freeCells.length);
      const key = freeCells.splice(index, 1)[0];
      this.dungeon[key] = '*';
  ...
  _generateWeapons: function _generateWeapons(freeCells) {
    WEAPON_TYPES.forEach((weapon) => {
      if (weapon.name === 'Club') { return; }
      const char = '' + weapon.name[0];
      const index = Math.floor(ROT.RNG.getUniform() * freeCells.length);
      const key = freeCells.splice(index, 1)[0];
  ...
  _drawWholeMap: function _drawWholeMap() {
    for (let key in this.dungeon) {
      ...
      const x = Number(cords[0]);
      const y = Number(cords[1]);
...
Player.prototype._draw = function _draw() {
  Map.display.draw(this._x, this._y, '@', 'black');
};

/* Player function to handle user keypresses and movement around map.
   Given control by React prop */
Player.prototype.handleEvent = function handleEvent(event) {
  ...
  const newX = this._x + direction[0];
  const newY = this._y + direction[1];
  ...
  const fovCallback = function fovCallback(x, y) {
    const key = x + ',' + y;
    if (key in Map.dungeon) { return (Map.dungeon[key] === 0); }
    return false;
  };

  field.compute(newX, newY, 10, (x, y, r, visibility) => Map.display.draw(x, y, Map.dungeon[key]));
};

/* Function for player interacting with items. results in a redix dispatch.
  Also updates the game messages */
Player.prototype._interact = function _interact() {
...
Player.prototype._fight = function _fight(newKey) {
  const x = Number(newKey.split(',')[0]);
  const y = Number(newKey.split(',')[1]);

  // get monsters, then filter out current monsters, then update store
  let monsters = dungeonStore.getState().monsters;
  const monster = monsters.filter((creature) => (creature._x === x && creature._y === y))[0];

  // remove the current monster from the monsters array
  monsters = monsters.filter((i) => !(i._x === x && i._y === y));

  // register the damage and / or death of monster then update the store
  const monsterDmg = monster._dmg;
  const userDmg = dungeonStore.getState().player.dmg();
  const userHealth = dungeonStore.getState().player.health;
...
/* *************** ROT Monster ********************** */
const Monster = function Monster(x, y, type) {
  this._x = x;
  this._y = y;
  this._type = type;
  this._draw();
...
Monster.prototype._draw = function _draw() {
  let color = '';
  let char = '';
...
const dungeonReducer = function dungeonReducer(state, action) {
  if (state === undefined){
    state = {
      player: {
        baseHealth: 50,
...
/* Redux Store */
const dungeonStore = Redux.createStore(dungeonReducer);
...
const Game = React.createClass({ displayName: 'Game',
  ...
  componentDidMount: function componentDidMount() {
    ...
    dungeonStore.subscribe(() => {
      const newState = self.props.getState();
...
ReactDOM.render(React.createElement(Game,
  { getState: dungeonStore.getState, dispatches: reduxDispatches, dungeon: Map }), document.getElementById('main')
);
